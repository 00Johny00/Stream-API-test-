...
public class Demo {
    ...
    System.out.println("----------FILTERED BOOK BY AUTHOR---------------");
    Arrays.stream(filteredBooksByAuthor).forEach(System.out::println);
    System.out.println("----------FILTERED BOOK BY PUBLISHER---------------");
    Book[] filterBooksByPublisher = bookService.filterBooksByPublisher(new Publisher(2, "Publisher_2"),books);
    Arrays.stream(filterBooksByPublisher).forEach(System.out::println);
    Book[] filteredBookAfterSpecifiedYear =  bookService.filterBooksAfterSpecifiedYear(1900,books);
    System.out.println("----------FILTERED BOOK BY YEAR---------------");
    Arrays.stream(filteredBookAfterSpecifiedYear).forEach(System.out::println);
    }
}
